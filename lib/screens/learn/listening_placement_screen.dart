// 聽力分級測驗作答畫面。流程與聽力測驗共用 QuizFlowController，
// 差異：不需選 mode/level、submit 走 PlacementService、
...
class _ListeningPlacementScreenState extends State<ListeningPlacementScreen> {
  ...
  late final QuizFlowController<PlacementResult> _flow = QuizFlowController(
    start: () async {
      final s = await PlacementService.startListeningPlacement();
    ...
    saveAnswer: (sessionId, questionId, optionId) =>
        PlacementService.answerListeningPlacement(
          sessionId: sessionId,
          questionId: questionId,
          selectedOptionId: optionId,
        ),
    submit: (sessionId, answers) =>
        PlacementService.submitListeningPlacement(sessionId, [
  ...
  Future<void> _confirm() async {
    ...
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
        builder: (_) => PlacementResultScreen(result: result, title: '聽力分級測驗'),
      ),
    );
  }
