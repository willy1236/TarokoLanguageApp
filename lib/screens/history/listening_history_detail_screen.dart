...
class ListeningHistoryDetailScreen extends StatefulWidget {
  ...
  const ListeningHistoryDetailScreen({
    super.key,
...
class _ListeningHistoryDetailScreenState
  ...
  late Future<ListeningHistoryDetail> _future;

  @override
  void initState() {
    super.initState();
    _future = HistoryService.fetchListeningDetail(widget.sessionId);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        bottom: false,
        child: FutureBuilder<ListeningHistoryDetail>(
          future: _future,
  ...
  Widget _buildContent(ListeningHistoryDetail detail) {
    return ListView(
      ...
      children: [
        Stack(
          children: [
            ScoreHeader(
              title: '${widget.level} · ${widget.modeLabel ?? '聽力測驗'}',
              score: detail.score,
              total: detail.total,
            ),
            Positioned(
