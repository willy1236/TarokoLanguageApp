...
PlacementResultItem _item({
  ...
  String? yours = '唱歌',
    ...
    PlacementResultItem(
      ...
      isCorrect: isCorrect,
      yourAnswer:
          yours == null ? null : ListeningAnswerRef(optionId: 2, text: yours),
...
PlacementResult _result({
  ...
  List<PlacementResultItem>? items,
    ...
    PlacementResult(
      ...
      results: items ??
          [
            _item(order: 1, isCorrect: true),
            _item(order: 2, isCorrect: false),
          ],
    );
...
void main() {
  ...
  testWidgets('明講建議可被覆蓋，不讓使用者以為被鎖等級', (tester) async {
    ...
    expect(
      find.text('這只是建議，你隨時可以手動選擇其他等級開始測驗。'),
      findsOneWidget,
    );
  });
  ...
  testWidgets('答對的題目不顯示「你的答案」，只顯示正確答案', (tester) async {
    await tester.pumpWidget(_app(_result(
      items: [_item(order: 1, isCorrect: true, correct: '教書')],
    )));
  ...
  testWidgets('沒作答的題目顯示（未作答）而不是空白', (tester) async {
    await tester.pumpWidget(_app(_result(
      items: [_item(order: 1, isCorrect: false, yours: null)],
    )));
  ...
  testWidgets('滿分也正常顯示', (tester) async {
    await tester.pumpWidget(_app(_result(
      score: 3,
      total: 3,
      items: [_item(order: 1, isCorrect: true)],
    )));
